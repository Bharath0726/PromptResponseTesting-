from collections import defaultdict   #Used for creating dictionaries with default values.
import traceback                      #Captures and displays detailed error information, useful for debugging
import openai                         #Interacts with the OpenAI API to utilize LLM models like GPT-4.
from openai import OpenAIError        #Specifically imports the OpenAIError exception class for handling OpenAI API errors
from tenacity import retry, stop_after_attempt, wait_random_exponential #Provides retry logic with exponential backoff
import tiktoken                       #A library for tokenizing text into tokens compatible with OpenAI's models. Used for chunking large contexts
...
def generate_csv_report(file, cols, criteria_dict, counter, config):
    try:
        ...
        progress_text = "Generating the. Please wait..."
