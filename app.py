...
st.markdown(
    "<h1 style='color:#4CAF50;text-align:center;'>🔮 Prompt Responses Testing Platform</h1>",
    unsafe_allow_html=True,
)

# Sidebar Configuration Section
st.sidebar.header("⚙️ Set Configuration")

config = {}

config["openai_api_key"] = st.sidebar.text_input(
    "🔑 OpenAI API Key", placeholder="sk-", type="password"
)
...
st.session_state["metrics_name"] = st.sidebar.multiselect(
     "📏 Select Metrics", ["Select All"] + all_metrics
)
...
uploaded_file = st.file_uploader(
    "📂 Upload a CSV file", help="Accept only .csv files", type="csv"
)
...
with col3:
    empty_button = st.button("🗑️ Clear Responses")
...
if click_button:
    try:
        ...
        for num in range(counter):
            ...
            if config["model_name"] in ["gpt-3.5-turbo-instruct"]:
